...
class SharedGNN(GaussianMixin, DeterministicMixin, Model):
...
class GraphNet(nn.Module):
    def __init__(self, hidden_features, out_features):
        ...
        self.input_layer1 = nn.Linear(22, self.input_features)
    ...
    def create_features(self,input):
        #Todo: make this more general
        object_feature = torch.cat([input[:,0:10],input[:,-12:]],dim=1)

        joint_feature = torch.zeros((input.shape[0],12,4), dtype=torch.float32, device=device)

        # for i in range(4):
        #     j = i
        #     joint_feature[:,i,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1],input[:,j+10+48:j+10+48+1]],dim=1)
        #
        # for i in range(4):
        #     j = i * 2 +4
        #     joint_feature[:,i+4,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1],input[:,j+10+48:j+10+48+1]],dim=1)
        #
        # for i in range(4):
        #     j = i * 2 +5
        #     joint_feature[:,i+8,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1],input[:,j+10+48:j+10+48+1]],dim=1)

        for i in range(4):
            j = i
            joint_feature[:,i,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1]],dim=1)

        for i in range(4):
            j = i * 2 +4
            joint_feature[:,i+4,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1]],dim=1)

        for i in range(4):
            j = i * 2 +5
            joint_feature[:,i+8,:] = torch.cat([input[:,j+10:j+10+1],input[:,j+10+12:j+10+12+1],input[:,j+10+24:j+10+24+1],input[:,j+10+36:j+10+36+1]],dim=1)
