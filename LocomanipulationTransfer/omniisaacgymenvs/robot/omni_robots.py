...
num_modules = 4
module_prefix_list = ['a1', 'a2', 'a3', 'a4']
...
# float_horizontal_robot_usd_path = os.path.join(root_ws_dir, "design", "RobotUSD", "bennett_legged1", "bennett_new.usd")
...
class OmniverseRobot(Robot):
    ...
    def prepare_base_contacts(self, stage, prim):
        for link_prim in prim.GetChildren():
            if link_prim.HasAPI(PhysxSchema.PhysxRigidBodyAPI): 
                if "base" in str(link_prim.GetPrimPath()):
                    rb = PhysxSchema.PhysxRigidBodyAPI.Get(stage, link_prim.GetPrimPath())
                    rb.CreateSleepThresholdAttr().Set(0)
                    cr_api = PhysxSchema.PhysxContactReportAPI.Apply(link_prim)
                    cr_api.CreateThresholdAttr().Set(0)
...
class QuadrupedRobot:
    ...
    def init_omniverse_robot(self, default_zero_env_path="/World/envs/env_0", sim_config=None):
        """
    ...
    def init_robot_articulation(self, scene):
        ...
        """
        arti_root_name = "/World/envs/.*/" + self.robot_name  + "/base"
        print(arti_root_name)
    ...
    def init_knee_view(self, scene):
        ...
        for i, module_prefix in enumerate(module_prefix_list):
            if i == 0:
                ...
                prefix_names += "|" + module_prefix
        knee_prim_path_expr = "/World/envs/.*/" + self.robot_name + "/({})_link[2,3]($|_right)".format(prefix_names)
    ...
    def init_tip_view(self, scene):
        ...
        tip_prim_path_expr = "/World/envs/.*/" + self.robot_name + "/({})_fingertip_frame".format(prefix_names)
